...
class FittsTestUI extends UIClass {
    ...
    newTrial() {
        ...
        if (this.trialCount > this.MAX_TRIALS) {
            ...
            this.childObjects = this.childObjects.filter(obj => obj !== this._theReticle && obj !== this._theTarget);
            // create new ones
            this.childObjects.push(this._theReticle  = new Reticle(retX, retY,this));
            this.childObjects.push(this._theTarget  = new Target(targX, targY, targDiam,this));
...
class Target extends ScreenObject{
    ...
    override draw(ctx : CanvasRenderingContext2D) : void {
        if (this.parentUI.currentState != "in_trial") return;
        // === YOUR CODE HERE ===
        ctx.beginPath();          
        // Draw the arc (circle)
    ...
    override pickedBy(ptX : number, ptY : number) : boolean {
        ...
        let d = Math.sqrt((ptX - this.centerX) ** 2 + (ptY - this.centerY) ** 2);
    ...
    override handleClickAt(ptX : number, ptY : number) : boolean {
        ...
        // === YOUR CODE HERE ===
        if (this.parentUI.currentState != "in_trial") return false
...
class Reticle extends Target {
    ...
    override draw(ctx : CanvasRenderingContext2D) : void {
        ...
        if (this.parentUI.currentState != "begin_trial") return;
    ...
    override handleClickAt(ptX : number, ptY : number) : boolean {
        ...
        // === YOUR CODE HERE ===
        if (this.parentUI.currentState != "begin_trial") return false
        // if the object is not visible and not picked, return false
        if (!this.visible || !this.pickedBy(ptX, ptY)) return false;
